# Templates

Templates answer: **How should the decision, test, or evidence be documented?**

- [Risk Assessment](risk-assessment-template.md)
- [Test Plan](test-plan-template.md)
- [Test Case](test-case-template.md)
- [Findings Report](findings-report-template.md)
- [Vendor Questionnaire Response](vendor-questionnaire-template.md)
- [Monitoring Plan](monitoring-plan-template.md)

Replace bracketed instructions and remove sections that are not applicable with a documented rationale.
